...
module.exports = function xml(json, opts,isHead) {;
	isHead=isHead==null?true:false;
	if(opts){
	...
	var result = isHead?'<xml>':'';
	type = json.constructor.name;

	if(type==='Array'){
		json.forEach(function(node){
			result += xml(node, opts,false);
		});
		...
		Object.keys(json).forEach(function(key){
			if(key!==opts.attributes_key){
				...
				if(opts.attributes_key && json[opts.attributes_key]){
					...
					result += util.format("<%s%s>%s</%s>", key, attributes, xml(node,opts,false), key);
			}
		});
	} else {
		return "<![CDATA["+json.toString().replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;')+"]]>";	
	}

	return result+(isHead==false?"":"</xml>");
};
